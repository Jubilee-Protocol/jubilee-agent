...
/**
     * DispatchAngelTool
     * Allows the agent to dispatch a specialized Sub-Agent to perform a task.
     */
export class DispatchAngelTool extends StructuredTool {
    name = 'dispatch_angel';
    description = 'Dispatch a specialized Sub-Agent to perform a complex task. Use this for parallel research, deep-dive coding, or verification steps that require focus.';

    schema = z.object({
        name: z.string().describe('Name of the Sub-Agent (e.g., "Research Specialist", "Coder").'),
        mission: z.string().describe('Detailed mission description.'),
        capabilities: z.array(z.enum(['web_search', 'browser', 'financial_metrics', 'read_filings', 'skill', 'fs_read', 'remember_fact', 'recall_memories'])).describe('Tools the Sub-Agent needs access to.'),
    ...
    async _call(arg: { name: string, mission: string, capabilities: string[], iterations: number }): Promise<string> {
        console.log(`\n🤖 Dispatching Sub-Agent: ${arg.name} for mission: "${arg.mission.slice(0, 50)}..."`);

        try {
            // 1. Configure Tools
            // We reuse getToolRegistry but filter by capabilities
            // Note: We avoid giving Sub-Agents "openclaw" system access (fs_write/shell) unless explicitly safe?
            ...
            const systemPrompt = `You are ${arg.name}, a specialized Sub-Agent.
